...
export async function handleRedirectCallback() {
  console.log("[Spotify] handleRedirectCallback started");
  console.log("[Spotify] Current URL:", window.location.href);

  const params = new URLSearchParams(window.location.search);
  const code = params.get("code");
  console.log("[Spotify] Code found:", !!code, code ? code.substring(0, 20) + "..." : "none");

  if (!code) {
    console.log("[Spotify] No code in URL, returning false");
    return false;
  }

  const verifier = sessionStorage.getItem(STORAGE.verifier);
  console.log("[Spotify] Verifier found:", !!verifier, verifier ? verifier.substring(0, 20) + "..." : "none");
  ...
  console.log("[Spotify] REDIRECT_URI used:", REDIRECT_URI);
  ...
  try {
    ...
    if (!response.ok) {
      ...
      console.error("[Spotify] Token error:", errBody);
      throw new Error(errBody?.error_description || "Spotify login failed.");
    }
    ...
    console.log("[Spotify] URL cleaned, returning true");
    return true;

  } catch (err) {
    console.error("[Spotify] handleRedirectCallback error:", err.message);
    history.replaceState({}, "", window.location.pathname);
    throw err;
  }
}
